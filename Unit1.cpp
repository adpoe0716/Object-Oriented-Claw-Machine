...
{
  for(int i=0;i<=3;i++){                  //夾爪生成
    jwas[i] = new Jwas(i);
  }
  for(int i=0;i<=6;i++){                   //娃娃生成
    doll[i] = new Doll(a);
...
Jwas::dow(int a){
if(a==1){T_Jwas->Top=T_Jwas->Top+3;
}else {T_Jwas->Height=T_Jwas->Height+1;}                                       //下
}
Jwas::Left_borad(){                                 //左邊界
...
Jwas::Right_borad(){
  if (jwas[3]->T_Jwas->Left + jwas[3]->T_Jwas->Width > 700){     //右邊界
...
TForm1::name(){
for(int i=0;i<=6;i++){                                             //娃娃名稱
if(i<=3){doll[i]->doll_name="協咖抱枕!";
}else if(i>3 && i<=5){doll[i]->doll_name="協咖公仔!";
}else{doll[i]->doll_name="摔哥公仔!";}}}

void __fastcall TForm1::Button1Click(TObject *Sender)               //投幣
{
  re();
  pay=1;
...
{
  Shape1->BringToFront();                              //黑洞
  if(money>=0 && money!=50){Label1->Caption=t1+t3+(50-money)+"元"+t2+t0;}     //文字輸出
  else if(money==50){Label1->Caption=t1+t4+t2+t0;}         //文字輸出(保夾)
  name();
  abcdef();
    ////////////
  ...
  ////////////////////
  if (GetAsyncKeyState(32)&0x8000 && pay==1  ){down=1;pay=0;}   //開夾
  if (down){
    for(int i=1;i<=3;i++){jwas[i]->dow(1);jwas[0]->dow(2); }//落下
    for(int i=0;i<=6;i++){
      int x1=doll[i]->T_Doll->Left;                                //娃娃宣告
      int x2=doll[i]->T_Doll->Left+doll[i]->T_Doll->Width;
      int y1=doll[i]->T_Doll->Top;
      int y2=doll[i]->T_Doll->Top +doll[i]->T_Doll->Height;
      ///////
      if((x1>=c1 && x1<=c2 && y1>=d1 && y1 <= d2)||             //左夾爪
      ...
      if((x1>=e1 && x1<=e2 && y1>=f1 && y1 <= f2)||             //右夾爪
  ...
  /////
  if (upp){         //上升
    for (int i=1;i<=3;i++){jwas[i]->up(1); jwas[0]->up(2);      //夾爪上升
    if (ha){doll[fo]->dup();}//娃娃上升
    if (jwas[1]->T_Jwas->Top<=38){    //鬆脫
         if(x<=4 && money!=50){doll_down=1;nha=1;}
        if(money==50){doll_down=0;}           //保夾
  ...
  if (doll_down && ha){          //鬆脫
    doll[fo]->doll_down();
    if (doll[fo]->T_Doll->Top>=doll_Top[fo]){doll_down=0; ha=0;}   //掉下去
  }//if
  ///////
  if (get){
     if (move_D && ha && !doll_down ){doll[fo]->doll_Right(); //娃娃右移
     if(doll[fo]->T_Doll->Left + doll[fo]->T_Doll->Width>=890){move_D=0;}}

     if (move_J){for (int i=0; i<=3 ;i++){jwas[i]->Right();     //夾爪右移
     if (jwas[1]->T_Jwas->Left>=778){move_J=0;L1=1;R1=1;}}}

     if (L1){jwas[2]->T_Jwas->Left=jwas[2]->T_Jwas->Left-1;      //鬆左爪
     if (jwas[2]->T_Jwas->Left<=769 ){L1=0;}}

     if(R1){jwas[3]->T_Jwas->Left=jwas[3]->T_Jwas->Left+1;       //鬆右爪
     if(jwas[3]->T_Jwas->Left>=891){R1=0;}}

     if(!move_D &&!move_J &&ha && !L1 && !R1 && !(doll[fo]->T_Doll->Top+doll[fo]->T_Doll->Height>=1030))
       {doll[fo]->doll_down();}     //娃娃掉落

     if(ha&&(doll[fo]->T_Doll->Top+doll[fo]->T_Doll->Height>=1030)){        //結果顯示
       get=0;gg=1;t2="\n恭喜夾到了"+doll[fo]->doll_name; money=0;
  ...
  if(gg&&!(jwas[1]->T_Jwas->Left<=280)){for(int i=0;i<=3;i++){jwas[i]->Left();}  //復位
